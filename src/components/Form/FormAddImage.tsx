...
export function FormAddImage({ closeModal }: FormAddImageProps): JSX.Element {
  ...
  const formValidations = {
    image: {
      ...
      validate: {
        lessThan10MB: fileList => fileList[0].size < 10485760 || 'O arquivo deve ser menor que 10MB',
        acceptedFormats: fileList =>
          /(?:([^:/?#]+):)?(?:([^/?#]*))?([^?#](?:jpeg|gif|png))(?:\?([^#]*))?(?:#(.*))?/g.test(fileList[0].type) ||
  ...
  const mutation = useMutation(async formData => api.post('/api/images', formData), {
  ...
  const onSubmit = async (data: FormValues): Promise<void> => {
    try {
      ...
      const response = await mutation.mutateAsync({ ...data, url: imageUrl } as unknown as void);
